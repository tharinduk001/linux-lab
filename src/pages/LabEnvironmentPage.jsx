    ...
    if (activeTask && !activeTask.completed) {
      ...
      if (!validationCommand) {
        toast({
          ...
          description: "This task does not have an automated validation command. Completing manually.",
          variant: "warning",
        });
          ...
          <TabsContent value="terminal" className="flex-grow mt-0 flex flex-col">
             <div className="flex-grow">
                <TerminalEmulator 
                    key={terminalKey} // Force re-mount on task change
                    task={activeTask} 
                    // onCommandSuccess is effectively replaced by WS message handling from validationResult
                    onData={handleTerminalInput} // Callback to send data via WebSocket
                    initialOutput={terminalOutputBuffer} // Pass buffered WS messages
                    isConnected={isWsConnected} // Pass WebSocket connection status
                    tasks={tasks} 
                    currentTaskIndex={currentTaskIndex}
                />
             </div>
            {activeTask && !activeTask.completed && (
              ...
              >
                <Button 
                  onClick={handleVerifyAndComplete} 
                  className="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-3 text-lg glow-effect"
                  disabled={!activeTask || activeTask.completed || !isWsConnected}
                >
                  <CheckCircle className="mr-2 h-5 w-5" /> Verify & Complete Task
                </Button>
              </motion.div>
